...
class MockHandler implements \Countable
{
    ...
    public function __invoke(\PNO\Geocoder\Vendor\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        $response = $response instanceof \Exception ? \PNO\Geocoder\Vendor\GuzzleHttp\Promise\rejection_for($response) : \PNO\Geocoder\Vendor\GuzzleHttp\Promise\promise_for($response);
        return $response->then(function ($value) use($request, $options) {
            ...
            if (isset($options['sink'])) {
                ...
                } elseif ($sink instanceof \PNO\Geocoder\Vendor\Psr\Http\Message\StreamInterface) {
                    $sink->write($contents);
                }
            }
            return $value;
