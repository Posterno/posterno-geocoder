...
function parse_request($message)
{
    ...
    $request = new \PNO\Geocoder\Vendor\GuzzleHttp\Psr7\Request($parts[0], $matches[1] === '/' ? _parse_request_uri($parts[1], $data['headers']) : $parts[1], $data['headers'], $data['body'], $version);
...
function parse_response($message)
{
    ...
    $parts = \explode(' ', $data['start-line'], 3);
    return new \PNO\Geocoder\Vendor\GuzzleHttp\Psr7\Response($parts[1], $data['headers'], $data['body'], \explode('/', $parts[0])[1], isset($parts[2]) ? $parts[2] : null);
}
/**
