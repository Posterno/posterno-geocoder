...
{
    return $reason instanceof \Exception || $reason instanceof \Throwable ? $reason : new \PNO\Geocoder\Vendor\GuzzleHttp\Promise\RejectionException($reason);
}
/**
...
function some($count, $promises)
{
    $results = [];
    $rejections = [];
    return \each($promises, function ($value, $idx, \PNO\Geocoder\Vendor\GuzzleHttp\Promise\PromiseInterface $p) use(&$results, $count) {
        if ($p->getState() !== \PNO\Geocoder\Vendor\GuzzleHttp\Promise\PromiseInterface::PENDING) {
            return;
        }
        $results[$idx] = $value;
...
{
    return (new \PNO\Geocoder\Vendor\GuzzleHttp\Promise\EachPromise($iterable, ['fulfilled' => $onFulfilled, 'rejected' => $onRejected]))->promise();
}
/**
...
{
    return (new \PNO\Geocoder\Vendor\GuzzleHttp\Promise\EachPromise($iterable, ['fulfilled' => $onFulfilled, 'rejected' => $onRejected, 'concurrency' => $concurrency]))->promise();
}
/**
...
{
    return each_limit($iterable, $concurrency, $onFulfilled, function ($reason, $idx, \PNO\Geocoder\Vendor\GuzzleHttp\Promise\PromiseInterface $aggregate) {
        $aggregate->reject($reason);
    });
}
/**
...
{
    return new \PNO\Geocoder\Vendor\GuzzleHttp\Promise\Coroutine($generatorFn);
}
